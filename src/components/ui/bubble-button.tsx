...
const Bubbles = () => {
  const [bubbleProps, setBubbleProps] = React.useState<Array<{
    width: number;
    height: number;
    left: number;
    duration: number;
  ...
  // Generate random bubble properties only on the client side
  React.useEffect(() => {
        ...
        {bubbleProps.map((props, i) => (
          <span
            ...
            style={{
              ...
              left: `${props.left}%`,
              animation: `rise ${props.duration}s ${props.delay}s linear infinite`,
            }}
          />
        ))}
      </div>
...
const UsageBadge = React.forwardRef<HTMLDivElement, UsageBadgeProps>(
  ({ icon, planName, usage, limit, tooltipContent, className }, ref) => {
    ...
    const usagePercentage = limit > 0 && usage !== Infinity ? (usage / limit) * 100 : 0;
          ...
          <TooltipTrigger asChild>
            ...
            >
              ...
              <div className="z-10">
                <span>{planName}</span>
                <span className="ml-2">
                  {usage === Infinity ? "∞" : usage}/{limit === Infinity ? "∞" : limit}
                </span>
              </div>
