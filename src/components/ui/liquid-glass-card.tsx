...
export const LiquidGlassCard = ({
  ...
  const MotionComponent = expandable ? motion.div : 'div';

  const motionProps: Record<string, unknown> = {};
  
  if (expandable) {
    motionProps.variants = containerVariants;
    motionProps.animate = isExpanded ? 'expanded' : 'collapsed';
    motionProps.onClick = handleToggleExpansion;
  ...
  return (
    <>
      ...
      <MotionComponent
        className={cn(
          `relative ${expandable ? 'cursor-pointer' : ''}`,
          className
        )}
        style={{
          borderRadius,
